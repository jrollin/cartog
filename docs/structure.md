...
- **[cartog-languages](../crates/cartog-languages/README.md)** — `Extractor` trait + 17 code extractors (Python, TypeScript (.ts, .tsx), JavaScript, Rust, Go, Ruby, Java, C, C++, C#, PHP, Dart, Swift, Kotlin, Vue, Svelte, Astro) + Markdown extractor. SFC extractors (Vue/Svelte/Astro) slice the `<script>` block and delegate to the JS/TS extractor with offset remapping.
