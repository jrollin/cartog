...
│
├── cartog-registry  (tier 1 — process-lock only; state dir + slots + registry)
│
...
- **[cartog-languages](../crates/cartog-languages/README.md)** — `Extractor` trait + 17 code extractors (Python, TypeScript (.ts, .tsx), JavaScript, Rust, Go, Ruby, Java, C, C++, C#, PHP, Dart, Swift, Kotlin, Vue, Svelte, Astro) + Markdown extractor. SFC extractors (Vue/Svelte/Astro) slice the `<script>` block and delegate to the JS/TS extractor with offset remapping, plus one whole-file `component` symbol per file.
...
- **[cartog-mcp](../crates/cartog-mcp/README.md)** — MCP server over stdio (`rmcp`). 17 tool handlers, single-writer election (primary + read-only attach + promotion).
...
- **[cartog-registry](../crates/cartog-registry/README.md)** — user-global state: platform state-directory resolution (`default_state_dir`), PID-lock slot derivation (`slot_for_db`), live-peer detection, and the machine-local project registry (`projects.sqlite`) that lets one session discover the other indexed projects on the machine. Owns everything per-user rather than per-project; no `cartog-db` dependency. See [explanation/project-registry.md](explanation/project-registry.md).
- **[cartog](../crates/cartog/README.md)** — binary crate: 28 top-level CLI commands via clap (including `cartog trace`/`context` for call paths and task-context bundles, `cartog push`/`pull` for S3 index sync, `cartog self update/version/rollback/migrate-db`, and `cartog projects list/add/scan/forget/prune` over the machine-local project registry), config resolution, logging, tokio runtime for `cartog serve`, daily background update probe.
