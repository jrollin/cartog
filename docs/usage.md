...
- **SessionEnd** (`update_on_exit.sh`): applies a deferred update at the safe boundary. For modern binaries (>= 0.14.0) it runs `cartog self update --apply-pending` once the serve process has exited (no-op if nothing was armed; skipped while a RAG pipeline lock is active; exit 6 means a peer is still live and it retries next session). For pre-0.14.0 binaries (which lack `cartog self update`) it still upgrades via `install.sh` — that legacy path is transitional and scheduled for removal.
