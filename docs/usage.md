...
- **SessionStart** (`drift_notice.sh`): non-blocking, runs alongside `ensure_indexed.sh` on the same `startup` matcher. `ensure_indexed.sh`'s drift line is plain stdout, which Claude Code feeds to the model but never shows in the terminal, so this script exists purely to put a **user-visible** notice on screen. It prints exactly one JSON object (`systemMessage` shown to the user, `additionalContext` reaching the model) or nothing at all. Silent unless all hold: `CARTOG_NO_UPDATE_CHECK` is unset, the plugin manifest has a version and `cartog` is on PATH, the project is cartog-configured (same consent gate as `ensure_indexed.sh`), the installed version is behind the pin, no update to exactly that pin is already armed, and the same (installed, pin) pair was not announced in the last 24 hours (a marker file in the cartog cache dir). A cargo-installed binary gets: "cartog X is older than the cartog plugin (Y) and was installed via cargo, so it cannot auto-update. Run: cargo install cartog --force". Everyone else gets: "cartog X is older than the cartog plugin (Y). It updates automatically when this session ends; to update now, run /cartog-install." An unconfigured project gets no cartog output of any kind.
