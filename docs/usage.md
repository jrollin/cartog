...
**Progress notifications**: `cartog_index` and `cartog_rag_index` emit standard MCP `notifications/progress` when the client includes a `progressToken` in the request's `_meta`. `cartog_index` emits a `walking` event, then per-file ticks for `parsing P/T files` and `storing P/T files`, plus `resolving edges P/T files` when the LSP pass runs — so larger repos produce more events. `cartog_rag_index` emits `preparing`, one `embedding processed/total` per ~512-symbol batch, then `storing`. The `message` field is human-readable, not a contract. Clients that do not supply a `progressToken` see no notifications and behavior is unchanged. Cold-cache or `force=true` runs report larger `total` values than warm runs; `total` is per-request, not historical.
