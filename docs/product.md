...
Measured across 13 scenarios, 9 languages. Best gains on call chain tracing (88% token reduction) and caller lookup (95% reduction).
...
**vs language servers (LSP):** no startup time, no per-language server, no config. Single binary covers 11 languages (10 code languages + Markdown). Trade-off: ~90% name resolution accuracy vs LSP's full semantic analysis. LSP can be enabled as an optional precision layer.
