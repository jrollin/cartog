...
run_scenario_php() {
    local fixture_name="webapp_php"
    local fixture_dir="$BENCH_DIR/fixtures/$fixture_name"
    local symbol="TokenError"

    should_skip_fixture "$fixture_name" && return 0

    echo -e "  ${CYAN}[$fixture_name]${NC} References to $symbol (extends, throw, catch)?" >&2
    ...
    # ── Best-effort grep: filter by context ──
    run_grep "best" "$fixture_dir" "grep -rn 'class.*extends.*$symbol\|throw new $symbol\|catch.*$symbol' ."
    local best_tok=$GREP_TOKENS
    local best_out="$GREP_OUTPUT"
    local best_cmds=1
...
run_scenario_java() {
    local fixture_name="webapp_java"
    local fixture_dir="$BENCH_DIR/fixtures/$fixture_name"
    local symbol="TokenException"

    should_skip_fixture "$fixture_name" && return 0

    echo -e "  ${CYAN}[$fixture_name]${NC} References to $symbol (extends, throws, catch)?" >&2
    ...
    # ── Best-effort grep: filter by context ──
    run_grep "best" "$fixture_dir" "grep -rn 'class.*extends.*$symbol\|throws.*$symbol\|throw new $symbol\|catch.*$symbol' ."
    local best_tok=$GREP_TOKENS
    local best_out="$GREP_OUTPUT"
    local best_cmds=1

    # ── Cartog: all refs to TokenException ──
    run_cartog_cmd "$fixture_dir" refs "$symbol"
    local cartog_tok=$CARTOG_TOKENS
    local cartog_out="$CARTOG_OUTPUT"
    local cartog_cmds=1
...
run_scenario_swift() {
    local fixture_name="webapp_swift"
    local fixture_dir="$BENCH_DIR/fixtures/$fixture_name"
    local symbol="TokenError"

    should_skip_fixture "$fixture_name" && return 0

    echo -e "  ${CYAN}[$fixture_name]${NC} References to $symbol (signature, throw)?" >&2
    ...
    # ── Best-effort grep: filter by context ──
    run_grep "best" "$fixture_dir" "grep -rn ': $symbol\|-> $symbol\|throw $symbol' ."
    local best_tok=$GREP_TOKENS
    local best_out="$GREP_OUTPUT"
    local best_cmds=1
...
run_scenario_cs() {
    local fixture_name="webapp_csharp"
    local fixture_dir="$BENCH_DIR/fixtures/$fixture_name"
    local symbol="TokenException"

    should_skip_fixture "$fixture_name" && return 0

    echo -e "  ${CYAN}[$fixture_name]${NC} References to $symbol (base list, throw)?" >&2
    ...
    # ── Best-effort grep: filter by context (C# `: Base`, `throw new`) ──
    run_grep "best" "$fixture_dir" "grep -rn ': $symbol\|throw new $symbol' ."
    local best_tok=$GREP_TOKENS
    local best_out="$GREP_OUTPUT"
    local best_cmds=1
...
run_scenario_cpp() {
    local fixture_name="webapp_cpp"
    local fixture_dir="$BENCH_DIR/fixtures/$fixture_name"
    local symbol="TokenError"

    should_skip_fixture "$fixture_name" && return 0

    echo -e "  ${CYAN}[$fixture_name]${NC} References to $symbol (base clause, throw)?" >&2
    ...
    # ── Best-effort grep: filter by context (C++ `: public Base`, `throw`) ──
    run_grep "best" "$fixture_dir" "grep -rn ': public $symbol\|: $symbol\|throw $symbol\|throw new $symbol' ."
    local best_tok=$GREP_TOKENS
    local best_out="$GREP_OUTPUT"
    local best_cmds=1
...
# C has no inheritance and no exceptions, so TokenError is referenced only
# through function signatures and returns — a type-reference row, not a
# base-clause one.
run_scenario_c() {
    local fixture_name="webapp_c"
    local fixture_dir="$BENCH_DIR/fixtures/$fixture_name"
    local symbol="TokenError"

    should_skip_fixture "$fixture_name" && return 0

    echo -e "  ${CYAN}[$fixture_name]${NC} References to $symbol (signature, return)?" >&2
    ...
    # ── Best-effort grep: filter to type contexts ──
    run_grep "best" "$fixture_dir" "grep -rn '$symbol \|$symbol\*\|enum $symbol\|return $symbol' ."
    local best_tok=$GREP_TOKENS
    local best_out="$GREP_OUTPUT"
    local best_cmds=1
