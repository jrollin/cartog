...
16 tools are exposed by default via rmcp's `#[tool_router]` macro with auto-generated JSON Schema parameters (18 with `CartogServer::with_federated(true)`, which adds the two cross-project tools `cartog_list_projects` and `cartog_search_all`; see `ServerOptions::federated`). Each tool also carries `annotations` (a `title` and `readOnlyHint`: on the default surface, `true` for the 13 query tools, `false` for the two index tools and `cartog_update`; the two federated tools are read-only too), and the read tools declare an `outputSchema`:
