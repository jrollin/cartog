...
| `ExcludeGlobs` | Compiled `[index] exclude` path globs passed to `index_directory()` (`empty()` for no extra exclusions; `from_globs()` errors on a malformed glob). Matches files and prunes matched directories. |
