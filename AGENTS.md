...
- **Index exclude globs**: `[index] exclude = [globs]` skips repo-root-relative paths during indexing, on top of the hardcoded dep/build-dir prune list (`is_ignored_dirname`). Compiled to a `globset::GlobSet` in `cartog_indexer::ExcludeGlobs` (`from_globs` errors on a bad glob → config rejection); `config::to_index_exclude` builds it, threaded into `index_directory` (9th arg) and `WatchConfig`. Matches files and directories; a matched dir is pruned via a synthetic-child probe (`dir/**` does not match the bare dir in globset). Honored by both the indexer walk and the watcher's `is_relevant_path`, so watch and index agree on scope. Cartog-specific filter; does not read `.gitignore`. See [docs/usage.md](docs/usage.md)
