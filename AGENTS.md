...
**Machine-readable endpoints.** The site also serves plain-text copies for coding agents, all *generated* from canonical sources so they cannot drift: `/llms.txt` (short index, hand-written in `src/pages/llms.txt.ts`), `/llms-full.txt` (full index, generated from `docs/README.md`'s bullet lists via `src/lib/docs.ts`), `/index.md` (serves `README.md`) and `/usage.md` (serves `docs/usage.md`). Relative links in the twins are rewritten to GitHub source — `blob/` for documents, `raw.githubusercontent.com` for images, since a `blob/` URL serves HTML not bytes — and a link escaping the repo root throws at build time rather than emitting a plausible 404. Each HTML page advertises its twin via `<link rel="alternate">`. `npm run build` runs `scripts/verify-machine-pages.mjs`, which asserts every endpoint emitted, non-trivially sized, free of unrewritten relative links, and advertising only pages and anchors that exist; adding an endpoint means adding it to that script's `MIN_BYTES`. Adding a new *page* means adding it to `PAGES` in `llms-full.txt.ts`.

**Site scope decisions** (settled 2026-09-08, after auditing graphify.com; do not relitigate without new evidence): `usage.astro` stays **one page**. Splitting it into `/mcp`, `/cli`, `/config`, `/provenance` was measured and rejected — 43 of its 67 internal links cross the proposed boundaries (24 alone from the walkthrough into the CLI reference), 11 anchors (quick-start, search-modes, workflow, languages, performance, troubleshooting, agent-skill, teach-agent, token-budget, json-output, compact-json) belong to none of the four, Pagefind indexes a single `data-pagefind-body`, and `usage.html` is referenced from 11 files including the VS Code extension source. It is one densely cross-referenced document, not four topics sharing a file. The SEO motive is largely served by the machine-readable endpoints above. The site also deliberately does **not** ship a page per language, a page per competitor, or persona pages ("for engineering leaders") — thin pages dilute, comparison pages age badly and invite a response, and there is no enterprise tier to funnel into. The landing page leads with the **mechanism** before any statistic, and edge provenance is sold as the trust model, since that plus hybrid search are the two things a parse-only competitor cannot claim.
...
- **Deferred self-update**: inside a Claude Code session the MCP server holds the serve lock, so `cartog self update` would refuse (exit 6). `cartog self update --defer` (or the `cartog_update` MCP tool) arms a pending update without swapping; the SessionEnd hook runs `--apply-pending` once the peer exits. The SessionStart hook now also arms the pin on drift (not only SessionEnd), so a session that never ends cleanly still converges; the apply itself stays deferred to the next boundary. A cargo-installed binary is never armed (`--defer` refuses it, exit 3) and converges only via `cargo install cartog --force`. A separate SessionStart hook, `drift_notice.sh`, shows the user a once-a-day, user-visible notice when the binary trails the pin, but only in a cartog-configured project. `plugin.json` forwards `CLAUDE_PLUGIN_ROOT` to the MCP server so `cartog serve` can resolve the pin (`ServerOptions::plugin_pin`) and surface it in its instructions and `cartog_stats`. See [docs/updates.md](docs/updates.md).
